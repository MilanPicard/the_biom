...
function update_selected_signatures_store(checkbox_value, cy_selected_nodes, checkbox_ids) {
    if (dash_clientside.callback_context.triggered_id == "overview_graph") {
        ...
        return {
            "triggered": dash_clientside.callback_context.triggered_id,
            "values": values
        }
    } else {
        return {
...
window.dash_clientside = Object.assign({}, window.dash_clientside, {
    clientside: {
        ...
        show_rendering_indicator: function (selectedNodeData) {
            const indicator = document.getElementById("rendering_indicator");
            if (indicator && selectedNodeData && selectedNodeData.length > 0) {
                indicator.style.display = "block";
            }
            return window.dash_clientside.no_update;
        },
        hide_rendering_indicator: function (elements) {
            const indicator = document.getElementById("rendering_indicator");
            if (indicator) {
                // Use requestAnimationFrame + setTimeout to wait for rendering to complete
...
function addEdgeHoverEffect(graphId) {
    ...
    if (graphId === 'overview_graph') {
        ...
        // Add box selection filter to only select nodes, not edges
        cy.off('boxselect'); // Remove previous listener to avoid duplicates
        cy.on('boxselect', function (evt) {
            const target = evt.target;
            // If an edge was selected during box selection, unselect it
            if (target.isEdge()) {
                target.unselect();
            }
        });
...
function addNodeHoverEffect(graphId) {
    const cy = document.getElementById(graphId)?._cyreg?.cy;
    if (!cy) return;
    // Remove previous listeners to avoid duplicates
    ...
    if (graphId === 'overview_graph') {
        // For the overview graph, add hover class to indicate clickability
        cy.on('mouseover', 'node', function (evt) {
            const node = evt.target;
            // Don't add hover effect to fake nodes
            if (!node.data('fake')) {
...
function attachAllNodeHoverEffects() {
    addNodeHoverEffect('overview_graph');
}

// Attach on window load
window.addEventListener('load', function () {
    setTimeout(function () {
        attachAllEdgeHoverEffects();
        attachAllNodeHoverEffects();
    }, 1000); // Delay to ensure Cytoscape is initialized
});
